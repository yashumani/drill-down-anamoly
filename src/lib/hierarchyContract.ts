...
export interface HierarchyForest { roots: HierarchyTreeNode[]; nodes: HierarchyTreeNode[]; nodesById: Map<string, HierarchyTreeNode>; nodeCount: number; valid: boolean; hasCycle: boolean; maxDepth: number; errors: string[]; warnings: string[]; issues: HierarchyIssue[]; }
...
export function buildHierarchyForest(rows: NormalizedHierarchyRow[]): HierarchyForest {
  ...
  // Surface structural problems in the visible warning stream as well as the strict error list.
  // The UI can explain why a tree is incomplete without hiding the fact that the contract is invalid.
  const warnings = issues.map((issue) => issue.message);
  const hasCycle = issues.some((issue) => issue.code === 'cycle');
  return { roots, nodes: [...nodesById.values()], nodesById, nodeCount: nodesById.size, valid: errors.length === 0 && roots.length > 0, hasCycle, maxDepth, errors, warnings, issues };
}
