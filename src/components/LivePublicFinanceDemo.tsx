...
export function LivePublicFinanceDemo() {
  ...
  return <section className="live-public-demo live-presentation" aria-label="Live multi-million-row public finance presentation">
    <header className="live-demo-head live-presentation-head">
      <div><span className="eyebrow">LIVE 3.8M-ROW FINANCE DECK</span><h2>City of Los Angeles procurement payments</h2><p>Navigate page by page. Every KPI, trend, flat-dimension view, and hierarchy branch is recalculated for the selected source scope and drill cohort.</p></div>
    ...
    <div className="live-stage">
      ...
      {result && !loading && slide === 'method' && <section className="live-slide-page live-method-page">
        ...
        <div className="live-method-grid"><section><h4>Why this demonstrates scale</h4><p>The full source count is retrieved live. Monthly, flat-dimension, hierarchy-branch, waterfall, and heatmap results are calculated from small server-side aggregates, so the browser does not hold millions of raw payment records.</p><p>The source contains {result.source.columnCount} columns; this presentation models ten flat finance dimensions and a separate ten-level analytical hierarchy.</p></section><section><h4>Interpretation limits</h4>{result.warnings.map((warning) => <p key={warning}>{warning}</p>)}</section><section><h4>Bring your own data</h4><p>Use one metric per file with period_date, actual_value, optional plan_value/forecast_value, metric metadata, and dim_* business dimensions. For an org chart, add node_id and parent_node_id; node_label and level_name are recommended.</p><button type="button" className="quiet-button" onClick={downloadFinanceDataTemplate}>Download Contract v1 template</button><p><a href={result.source.datasetUrl} target="_blank" rel="noreferrer">Open the official dataset</a></p><p><a href={result.source.apiDocsUrl} target="_blank" rel="noreferrer">Open Socrata API documentation</a></p></section></div>
      </section>}
    </div>
