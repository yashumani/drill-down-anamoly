...
export function CombinationExplorer({ interactions, onDrill }: { interactions: InteractionSegment[]; onDrill: (predicates: Predicate[]) => void }) {
  ...
  return <section className="combination-explorer" id="combined-drivers" aria-label="Combined driver analysis">
    <div className="combination-explorer-head"><div><span className="deck-kicker">COMBINED DRIVERS</span><h3>Which values become important only when they occur together?</h3><p>Each bubble is one supported group. Higher lift means the pattern is more concentrated than normal. Above zero is favorable; below zero is unfavorable. Bubble size represents record support.</p></div><div className="combination-legend"><span>Horizontal = concentration</span><span>Vertical = business impact</span><span>Size = support</span></div></div>
