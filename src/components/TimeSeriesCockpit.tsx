...
export function TimeSeriesCockpit({
  ...
  return <section className="time-cockpit" aria-label="CFO pulse and finance time intelligence">
    <div className="time-cockpit-head">
      ...
      <button type="button" title="Download the current time-series configuration, evidence, warnings, and calculation identifiers as JSON." className="quiet-button" onClick={() => downloadSnapshot(result)}>Export analysis snapshot</button>
    </div>

    <div className="time-controls" aria-label="Time intelligence controls">
      <label><span className="control-label">Time field <InfoTip text="The date or fiscal-period column used to place each row on the time axis." label="Time field" /></span><select value={timeField} onChange={(event) => onTimeField(event.target.value)}>{candidates.map((candidate) => <option key={candidate.field} value={candidate.field}>{humanize(candidate.field)} · {(candidate.parseRate * 100).toFixed(0)}% parsed</option>)}</select></label>
      <label><span className="control-label">Period grain <InfoTip text="Controls whether records are aggregated into days, weeks, months, or quarters." label="Period grain" /></span><select value={grain} onChange={(event) => onGrain(event.target.value as TimeGrain)}><option value="day">Daily</option><option value="week">Weekly</option><option value="month">Monthly</option><option value="quarter">Quarterly</option></select></label>
      <label><span className="control-label">Window <InfoTip text="Limits the displayed and analyzed reporting range, including MTD, QTD, YTD, and rolling windows." label="Time window" /></span><select value={window} onChange={(event) => onWindow(event.target.value as TimeWindow)}><option value="90d">Last 90 days</option><option value="8w">Last 8 weeks</option><option value="13w">Last 13 weeks</option><option value="15m">Last 15 months</option><option value="24m">Last 24 months</option><option value="mtd">MTD</option><option value="qtd">QTD</option><option value="ytd">YTD</option><option value="all">All available periods</option></select></label>
      <label><span className="control-label">Aggregation <InfoTip text="Sum is for flow metrics, Average is exploratory for averages or rates, and Period end is for balances or ending headcount." label="Aggregation method" /></span><select value={aggregation} onChange={(event) => onAggregation(event.target.value as AggregationMethod)}><option value="sum">Sum · flow metric</option><option value="average">Average · rate / average</option><option value="period_end">Period end · balance / headcount</option></select></label>
      <label><span className="control-label">Fiscal year starts <InfoTip text="Sets the fiscal calendar used for quarter-to-date and year-to-date calculations." label="Fiscal year start" /></span><select value={fiscalYearStartMonth} onChange={(event) => onFiscalYearStartMonth(Number(event.target.value))}>{monthNames.map((month, index) => <option key={month} value={index + 1}>{month}</option>)}</select></label>
      <label><span className="control-label">Materiality <InfoTip text="The minimum business-impact size, expressed as a percentage of plan, required before a period is treated as financially material." label="Materiality threshold" /></span><select value={materialityPercent} onChange={(event) => onMaterialityPercent(Number(event.target.value))}><option value={0.01}>1% of plan</option><option value={0.03}>3% of plan</option><option value={0.05}>5% of plan</option><option value={0.1}>10% of plan</option></select></label>
    </div>
    ...
    <div className="time-summary-grid">
      ...
      <article className={`time-metric health-${result.modelHealth.status}`}><span className="metric-label">Analysis health <InfoTip text="A readiness score based on time parsing, measure coverage, comparison coverage, history length, and monitoring diagnostics. It is not a probability." label="Analysis health" side="bottom" /></span><strong>{result.modelHealth.score.toFixed(0)}/100</strong><small>{humanize(result.modelHealth.status)} · {result.modelHealth.periodCount} periods</small></article>
      <article className={`time-metric health-${forecastBacktest.status === 'ready' ? 'healthy' : forecastBacktest.status}`}><span className="metric-label">Forecast backtest <InfoTip text="Out-of-sample comparison of simple baseline methods. The selected champion is descriptive evidence, not an approved enterprise forecast." label="Forecast backtest" side="bottom" /></span><strong>{forecastBacktest.champion ? humanize(forecastBacktest.champion) : 'Not ready'}</strong><small>{forecastChampion?.wape == null ? `${forecastBacktest.historyPeriods} history periods` : `WAPE ${(forecastChampion.wape * 100).toFixed(1)}% · bias ${forecastChampion.bias == null ? '—' : `${(forecastChampion.bias * 100).toFixed(1)}%`}`}</small></article>
    </div>
