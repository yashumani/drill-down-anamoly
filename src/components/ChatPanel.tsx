...
export function ChatPanel({ rows, dimensions, actualKey, expectedKey, metricPolarity, predicates, result, dataQuality, externalContext, manualContext, onExternalContext, onAction }: {
  ...
  const [messages, setMessages] = useState<Message[]>([{ role: 'assistant', text: 'Ask whether the data is trustworthy, what changed, why a group stands out, or what to investigate next.' }]);
  ...
  const starterSuggestions = useMemo(() => {
    ...
    const output = ['Is the data quality good enough?', ...demoQuestions.slice(0, 1), 'What is driving the result?', 'What is the strongest combined pattern?'];
  ...
  return <aside className="chat-panel" aria-label="Ask the data">
    <div className="chat-head"><div><span className="chat-kicker">ASK THE DATA</span><h2>Explain this result</h2></div><button className="icon-button" type="button" onClick={() => setSettingsOpen((value) => !value)}>{llm.enabled ? 'LLM on' : 'Connect LLM'}</button></div>
    ...
    {settingsOpen && <div className="llm-settings">
      ...
      <p className="security-note">The key remains only in this page's memory. Browser calls require CORS; production deployments should proxy approved LLM providers through a secured backend and enforce data-loss-prevention rules.</p>
    </div>}

    <details className="context-box">
      <summary>Add business context</summary>
      <p>Add events the dataset cannot see—campaigns, outages, launches, policy changes, weather, competitor moves, or operational incidents. News context is included automatically when available.</p>
      ...
      <textarea value={manualContext} onChange={(event) => onExternalContext(event.target.value)} placeholder="Example: West stores had a device shortage from July 8–18; Promo B launched July 10..." />
      {externalContext && <small className="context-status">External context is active for Ask the Data.</small>}
    </details>

    <div className="chat-messages" aria-live="polite">{messages.slice(-8).map((message, index) => <div key={`${index}-${message.role}`} className={`chat-message ${message.role}`}><span>{message.role === 'assistant' ? (llm.enabled ? 'AI analyst' : 'Data guide') : 'You'}</span><p>{message.text}</p></div>)}</div>
    ...
    <div className="chat-input-row"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') ask(input); }} placeholder="Ask: Is the data reliable enough?" aria-label="Ask a question about the data" /><button type="button" disabled={busy} onClick={() => ask(input)}>{busy ? 'Thinking…' : 'Ask'}</button></div>
  </aside>;
}
